# AUTO-SEPARATED from matching/version_detector.py for pipeline_url pipeline
...
* ``alternate`` — alternate official version (director's cut, etc.)
* ``uncensored`` — uncensored / uncut / unedited version
...
_ALTERNATE_KEYWORDS = re.compile(
    r"\b(?:alternate\s+version|alt\s+version|version\s+[2-9]|v[2-9]"
...
_UNCENSORED_KEYWORDS = re.compile(
    r"\b(?:uncensored|uncut|unedited)\b",
    re.IGNORECASE,
)
...
def detect_version_type(
    *,
    ...
    Args:
        ...
        hint_uncensored:        User indicated this is an uncensored version
    ...
    if hint_uncensored:
        signals.append(VersionSignal("user_hint", "uncensored", 0.90,
                                     "User indicated uncensored version"))
...
def _check_title_keywords(signals: List[VersionSignal], source_title: str, filename: str):
    ...
    for text, label in [(source_title, "source_title"), (filename, "filename")]:
        ...
        if _UNCENSORED_KEYWORDS.search(text):
            signals.append(VersionSignal(
                f"{label}_keywords", "uncensored", 0.75,
                f"Uncensored keyword found in {label}: {text}",
            ))
...
def _check_description_keywords(signals: List[VersionSignal], description: str):
    ...
    if _UNCENSORED_KEYWORDS.search(desc):
        signals.append(VersionSignal(
            "description_keywords", "uncensored", 0.50,
            "Uncensored keyword found in description",
        ))
...
def _aggregate_signals(
    ...
    type_scores: Dict[str, float] = {"cover": 0.0, "live": 0.0, "alternate": 0.0, "uncensored": 0.0, "normal": 0.0}
    for sig in signals:
    ...
    best_type = "normal"
    best_score = 0.0
    for vt in ("cover", "live", "alternate", "uncensored"):
