# AUTO-SEPARATED from ai/metadata_service.py for pipeline_url pipeline
# This file is independent â€” changes here do NOT affect the other pipeline.
"""
AI Metadata Service â€” Orchestrates AI-powered metadata enrichment.

Implements the complete two-phase workflow:

Phase 1 â€” Proposal Generation:
...
Phase 2 â€” Diff Review & Application:
...
# Minimum confidence to auto-apply a field (0.0â€“1.0)
...
def enrich_video_metadata(
    ...
    # â”€â”€ Step 1: Pre-AI mismatch detection â”€â”€
    ...
    # â”€â”€ Step 2: Audio fingerprinting (optional) â”€â”€
    ...
    # â”€â”€ Step 3: Get AI provider with model routing â”€â”€
    ...
    # â”€â”€ Step 4: Create AI result record â”€â”€
    ...
    try:
        # â”€â”€ Step 5: Call AI provider â”€â”€
...
def _auto_apply_fields(
    ...
    # When a field is currently empty, use a lower threshold â€” any AI data
    # is an improvement over nothing.
    EMPTY_FIELD_THRESHOLD = 0.5

    # Plot is the primary enrichment target â€” NFO plots are typically raw
    ...
    if (
        ...
        # or soundtracks â€” these are almost always false positives.
...
def apply_ai_fields(
    ...
    # â”€â”€ Create pre-change snapshot for undo â”€â”€
    genres = [g.name for g in video.genres] if video.genres else []
    snapshot_data = {
        "artist": video.artist,
        "title": video.title,
    ...
    # â”€â”€ Apply artwork (promote pending â†’ valid, remove old) â”€â”€
    ...
    # â”€â”€ Apply proposed source links â”€â”€
    ...
    if source_fields_requested and hasattr(ai_result, 'proposed_sources') and ai_result.proposed_sources:
        ...
        for sf in source_fields_requested:
            # e.g. "source:musicbrainz:single" â†’ ("musicbrainz", "single")
            parts = sf.split(":", 2)
            if len(parts) == 3:
...
def _rename_files_for_metadata(db: Session, video: VideoItem):
    ...
    try:
        ...
        logger.info(f"Renamed: {old_folder} â†’ {new_folder}")
    except Exception as e:
