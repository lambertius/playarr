# AUTO-SEPARATED from metadata/providers/musicbrainz.py for pipeline_url pipeline
# This file is independent â€” changes here do NOT affect the other pipeline.
"""
MusicBrainz Provider â€” Structured metadata from MusicBrainz.

Primary source for:
- Canonical names & MBIDs (artist, release, recording)
- Structured data (year, release date, genres, country)

Rate-limit: â‰¤1 req/sec â€” enforced via ``time.sleep(1.1)`` between calls.
"""
import logging
import time
...
from app.scraper.artist_album_scraper import _resolve_commons_url
...
class MusicBrainzProvider(MetadataProvider):
    """MusicBrainz metadata provider â€” canonical structured data."""
    ...
    def search_track(self, artist: str, title: str) -> List[ProviderResult]:
        ...
        for r in mb.get("recording-list", []):
            ...
            # Releases â€” pick the best single/EP release only
