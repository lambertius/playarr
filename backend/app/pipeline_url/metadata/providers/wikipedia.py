# AUTO-SEPARATED from metadata/providers/wikipedia.py for pipeline_url pipeline
# This file is independent â€” changes here do NOT affect the other pipeline.
"""
Wikipedia Provider â€” Scrapes Wikipedia for artist/album/track metadata.
...
Opportunistic â€” not all entities have Wikipedia pages.
"""
import logging
import re
...
def _scored_wiki_search(terms: List[str], positive_keywords: List[str],
    ...
    for term in terms:
        for r in _wikipedia_search_api(term, limit=5):
            ...
            # contains extra words beyond the search name â€” likely a
...
class WikipediaProvider(MetadataProvider):
    """Wikipedia metadata provider â€” opportunistic scraping."""
    ...
    def search_artist(self, name: str) -> List[ProviderResult]:
        ...
        # Wikipedia can redirect "Kazoo Kid" â†’ "Macaulay Culkin" etc.
    ...
    def get_artist(self, key: str) -> Optional[ProviderResult]:
        # Wikipedia doesn't have stable IDs â€” re-search
    ...
    def search_track(self, artist: str, title: str) -> List[ProviderResult]:
        ...
        if not _has_music_field:
            logger.info(
                ...
                f"fields â€” article is likely not about a song, discarding {url}"
            )
            return []
        ...
        if mismatch:
            logger.warning(
                f"WikipediaProvider.search_track: article mismatch â€” "
