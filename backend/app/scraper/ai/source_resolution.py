...
"""
AI Source Resolution â€” Determine canonical identity and external source links
BEFORE scrapers run.

This is the new first AI stage in the metadata pipeline:

  1. Parse local/source identity hints
  2. **AI Source Resolution** â† THIS MODULE
...
SOURCE_RESOLUTION_SYSTEM_PROMPT = (
    ...
    "Respond in valid JSON only â€” no markdown, no code fences, no extra text."
)
...
### Step 2 â€” Source Link Resolution
...
### Step 3 â€” Confidence Assessment
Rate your confidence:
- **identity**: How confident are you in the artist/title/album identification (0.0â€“1.0)
- **sources**: How confident are you that the source links are correct (0.0â€“1.0)

## RESPONSE FORMAT
Respond in **valid JSON only** â€” no markdown fences, no commentary:
{{
  "identity": {{
...
def _build_source_resolution_prompt(
    *,
    ...
    if fingerprint_artist or fingerprint_title:
        fingerprint_section = (
            f"\n**Audio fingerprint match:** {fingerprint_artist} â€” {fingerprint_title} "
...
def _parse_source_resolution_response(raw: str) -> SourceResolutionResult:
    ...
    try:
        ...
        # Try to extract JSON from the response, with truncation repair
        from app.scraper.ai.final_review import _try_parse_json
        data = _try_parse_json(text)
        if data is None:
            logger.warning(
                "Failed to parse AI source resolution response as JSON "
...
def resolve_sources_with_ai(
    *,
    ...
    if not provider:
        logger.info("AI source resolution skipped â€” no AI provider configured")
        return None
    ...
    logger.info(
        f"AI Source Resolution â€” sending prompt ({len(prompt)} chars) "
    ...
    try:
        ...
        # (we don't need the full enrich_metadata flow, just a text prompt â†’ JSON response)
...
# Models that are reasoning models â€” they reject temperature != 1 and top_p.
