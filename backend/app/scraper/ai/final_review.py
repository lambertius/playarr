...
"""
AI Final Review Ã¢â‚¬â€ Post-scraping verification and correction.
...
This is the second AI stage in the metadata pipeline (after source resolution):
  ...
  3. Validation
  4. **AI Final Review** Ã¢â€ Â THIS MODULE
  5. Save metadata
...
FINAL_REVIEW_SYSTEM_PROMPT = (
    ...
    "Respond in valid JSON only Ã¢â‚¬â€ no markdown, no code fences, no extra text."
)
...
**IMPORTANT Ã¢â‚¬â€ Album field for music videos:**
...
**CRITICAL Ã¢â‚¬â€ Album sentinel values:**
...
### Step 2 Ã¢â‚¬â€ Verify and Improve Description
The scraped description likely comes from Wikipedia and should be preserved and improved Ã¢â‚¬â€ NOT replaced with a short summary.

**CRITICAL Ã¢â‚¬â€ Platform description is NOT the plot:**
...
- Raw YouTube description text (contains URLs, social media links, lyrics, or production credits) Ã¢â‚¬â€ this should NEVER be used as the plot
...
def _try_parse_json(text: str) -> Optional[dict]:
    ...
    # 1b. Sanitize control characters and trailing commas, then retry.
    #     LLMs sometimes emit literal control chars inside string values
    #     (e.g. unescaped tabs) or trailing commas before } / ].
    ...
    # 1c. Repair bracket/brace mismatches.
    #     GPT-5 sometimes closes arrays with } instead of ] (or vice versa).
    #     Scan the JSON tracking a bracket/brace stack and fix mismatches.
    ...
    if repaired:
        ...
        try:
            ...
            logger.debug(f"JSON bracket-mismatch repair failed: {e}")
    ...
    if match:
        try:
            ...
            logger.debug(f"JSON extract-block parse failed: {e}")

    # 3. Try to repair truncated JSON (model hit output token limit).
    #    Find the outermost '{' and attempt to close open brackets/braces.
    first_brace = sanitized.find('{')
    if first_brace == -1:
        return None
    fragment = sanitized[first_brace:]

    # Strip trailing incomplete string (text ending mid-value)
    fragment = re.sub(r',\s*"[^"]*$', '', fragment)          # trailing key start
    fragment = re.sub(r',\s*"[^"]*":\s*"[^"]*$', '', fragment)  # trailing string value
    fragment = re.sub(r',\s*"[^"]*":\s*$', '', fragment)     # trailing key: no value
    ...
    # Close any unclosed strings (heuristic: odd quote count)
    quote_count = fragment.count('"') - fragment.count('\\"')
    if quote_count % 2 == 1:
        fragment += '"'

    fragment += ']' * max(0, sq_opens)
    fragment += '}' * max(0, opens)

    try:
        return json.loads(fragment)
    except json.JSONDecodeError as e:
        logger.debug(f"JSON truncation-repair parse failed: {e}")
        return None
...
def _parse_final_review_response(raw: str) -> FinalReviewResult:
    ...
    if data is None:
        # Log both the start and, when long, the area around the likely
        # error position so we can diagnose what the AI returned.
        import re as _re
        _snippet_end = raw[-200:] if len(raw) > 500 else ""
        logger.warning(
            "Failed to parse AI final review response "
            f"(length={len(raw)}, first 300 chars: {raw[:300]!r}"
            + (f", last 200 chars: {_snippet_end!r}" if _snippet_end else "")
            + ")"
        )
...
def run_final_review(
    *,
    ...
    if not provider:
        logger.info("AI final review skipped Ã¢â‚¬â€ no AI provider configured")
        return None
    ...
    logger.info(
        f"AI Final Review Ã¢â‚¬â€ sending prompt ({len(prompt)} chars) "
    ...
    try:
        ...
        from app.scraper.ai.source_resolution import _call_provider_raw
