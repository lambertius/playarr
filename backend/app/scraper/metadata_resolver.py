...
"""
Metadata Resolver Ã¢â‚¬â€ Scrape and resolve metadata from multiple sources.
...
def capitalize_genre(name: str) -> str:
    ...
    - Title-cases each word ("indie rock" Ã¢â€ â€™ "Indie Rock")
    - Preserves acronyms like R&B, EDM, IDM, DJ
    - Handles hyphenated genres ("lo-fi" Ã¢â€ â€™ "Lo-Fi", "post-punk" Ã¢â€ â€™ "Post-Punk")
    """
    if not name:
        return name
...
def extract_artist_title(raw_title: str) -> Tuple[str, str]:
    ...
    # Try dash-separated Ã¢â‚¬â€ only match separators outside brackets
    for separator in [" - ", " Ã¢â‚¬â€ ", " Ã¢â‚¬â€œ ", " : "]:
...
def _pick_best_release(
    ...
    def _sort_key(rel):
        ...
        # De-prioritize releases whose release-group carries a
        # "Compilation" secondary type — these are VA samplers that
        # happen to include the track, not the artist's own release.
        stypes = [s.lower() for s in rg.get("secondary-type-list", [])]
...
# Secondary types that indicate a release group is NOT a standard studio album.
...
def _find_parent_album(recording_id: str) -> Optional[Dict[str, Any]]:
    ...
    def _extract_album(rel_list, exclude_types=_EXCLUDED_SECONDARY_TYPES,
        ...
        for rel in rel_list:
            ...
            if rg_type not in primary_types:
                continue
    ...
    result = _extract_album(rel_list, exclude_types=_EXCLUDED_SECONDARY_TYPES - {"soundtrack"})
    if result:
        return result
    # Pass 3: accept EP release groups (for tracks only released on an EP)
    return _extract_album(rel_list, primary_types={"ep"})
...
def _find_album_by_artist_browse(
    ...
    for rg in browse.get("release-group-list", []):
        ...
        for medium in rel.get("medium-list", []):
            for track in medium.get("track-list", []):
                ...
                sim = SequenceMatcher(None, title_lower, rec_title_lower).ratio()
                ...
                if sim < _min_sim:
                    ...
                    _bc = _re_ab.sub(r"\s*\(.*?\)", "", rec_title_lower).strip()
                ...
                if sim >= _min_sim:
                    ...
                    if _is_soundtrack:
                        ...
                        break
                    logger.info(
                        f"Album candidate via artist browse: '{album_title}' contains "
    ...
    # Fallback: browse EP release groups for tracks only released on an EP
    try:
        ep_browse = musicbrainzngs.browse_release_groups(
            artist=mb_artist_id, release_type=["ep"], limit=50,
        )
        time.sleep(1.1)
        for rg in ep_browse.get("release-group-list", []):
            rg_id = rg.get("id")
            if not rg_id:
                continue
            secondary = {s.lower() for s in rg.get("secondary-type-list", [])}
            if secondary & _EXCLUDED_SECONDARY_TYPES:
                continue
            try:
                releases = musicbrainzngs.browse_releases(
                    release_group=rg_id, limit=5, includes=["recordings"],
                )
                time.sleep(1.1)
            except Exception:
                continue
            ...
            for medium in rel.get("medium-list", []):
                for track in medium.get("track-list", []):
                    ...
                    if sim >= _min_sim:
                        album_title = rg.get("title") or rel.get("title")
                        logger.info(
                            f"EP found via artist browse: '{album_title}' contains "
                            f"'{rec_title}' (sim={sim:.2f} for '{title}', "
                            f"rg={rg_id}, release={rel.get('id')})"
                        )
                        return {
                            "album": album_title,
                            "mb_album_release_id": rel.get("id"),
                            "mb_album_release_group_id": rg_id,
                        }
    except Exception as e:
        logger.warning(f"EP browse fallback for artist {mb_artist_id} failed: {e}")
...
def _find_release_group_by_name(
    mb_artist_id: str,
    album_name: str,
) -> Optional[Dict[str, Any]]:
    """Find a release group by name for a given artist.

    Browses the artist's release groups (album + EP types) and finds one
    whose title matches the given album name.  Used as a fallback when the
    album name is known (from AI, Wikipedia, or MB recording text) but the
    release-group ID could not be resolved through normal parent-album
    lookups — e.g. when ``_find_parent_album`` only checks Album-type
    releases and the track actually lives on an EP.
    """
    if not mb_artist_id or not album_name:
        return None
    ...
    # Strip trailing " ep" for flexible matching
    # ("Night Thinker EP" should match "Night Thinker")
    _ep_stripped = re.sub(r'\s+ep$', '', album_norm).strip()

    for rg_types in (["album", "ep"], ["single"]):
        try:
            browse = musicbrainzngs.browse_release_groups(
                artist=mb_artist_id, release_type=rg_types, limit=100,
            )
            time.sleep(1.1)
        except Exception as e:
            logger.warning(
                f"_find_release_group_by_name: browse failed for artist "
                f"{mb_artist_id} (type={rg_types}): {e}"
            )
            continue

        for rg in browse.get("release-group-list", []):
            rg_title = _norm(rg.get("title", ""))
            rg_stripped = re.sub(r'\s+ep$', '', rg_title).strip()

            sim = SequenceMatcher(None, album_norm, rg_title).ratio()
            sim_stripped = SequenceMatcher(None, _ep_stripped, rg_stripped).ratio()

            if (max(sim, sim_stripped) < 0.85
                    and album_norm != rg_title
                    and _ep_stripped != rg_stripped):
                continue
            ...
            try:
                ...
                for rel in releases.get("release-list", []):
                    ...
                    if caa.get("front") in (True, "true"):
                        ...
                        break
                if not release_id and releases.get("release-list"):
                    release_id = releases["release-list"][0].get("id")
            except Exception:
                pass

            logger.info(
                f"Release group found by name: '{rg.get('title')}' "
                f"(rg={rg_id}, release={release_id}, "
                f"sim={max(sim, sim_stripped):.2f})"
            )
            return {
                "album": rg.get("title"),
                "mb_album_release_id": release_id,
                "mb_album_release_group_id": rg_id,
            }

    logger.info(
        f"No release group found by name for '{album_name}' "
...
def _search_single_release_group(
    ...
    directly returns singles Ã¢â‚¬â€ unlike ``search_recordings`` whose
    ...
    mb_release_group_id, artist, title, album, year, genres Ã¢â‚¬â€ or None
    ...
    for search_type in ("single", "ep"):
        ...
        for rg in rg_list:
            ...
            if rtype not in ("single", "ep"):
                continue
            secondary = [s.lower() for s in rg.get("secondary-type-list", [])]
            # Skip live/session singles Ã¢â‚¬â€ source link should point to
            # the original single, not a live or session variant.
            if "live" in secondary:
                continue
            rg_title_lower = _norm((rg.get("title") or "").lower().strip())
            score = 0

            # Prefer singles over EPs Ã¢â‚¬â€ EPs are an acceptable fallback
            if rtype == "ep":
                score -= 20

            # Artist match is critical Ã¢â‚¬â€ extract candidate artist name
            ...
            if any(q in rg_title_lower for q in ["sped up", "slowed", "remix", "edit", "version", "acoustic", "live"]):
                if any(q in rg_title_lower for q in ["sped up", "slowed", "remix"]):
                    ...
                    score -= 10
            # Session variants (e.g. "Spotify Sessions") without secondary-type
            if any(q in rg_title_lower for q in ["session", "sessions"]):
                score -= 80
        ...
        for search_type in ("single", "ep"):
            ...
            for rg in rg_list:
                ...
                if rtype not in ("single", "ep"):
                    continue
                secondary = [s.lower() for s in rg.get("secondary-type-list", [])]
                # Skip live/session singles Ã¢â‚¬â€ same as primary loop
                if "live" in secondary:
                    continue
                rg_title_lower = _norm((rg.get("title") or "").lower().strip())
                score = 0
                ...
                if any(q in rg_title_lower for q in ["sped up", "slowed", "remix", "edit", "version", "acoustic", "live"]):
                    if any(q in rg_title_lower for q in ["sped up", "slowed", "remix"]):
                        ...
                        score -= 10
                # Session variants without secondary-type
                if any(q in rg_title_lower for q in ["session", "sessions"]):
                    score -= 80
                frd = rg.get("first-release-date", "")
                if frd and len(frd) >= 4:
    ...
    # releases (e.g. "Ida Corr vs. Fedde Le Grand"), but only when the
    # primary credit has some similarity — prevents split/collab singles
    # (e.g. "Iron & Wine / American Football") from passing when the
    # query artist is a secondary credit.
...
def search_musicbrainz(
    ...
    try:
        ...
        if artist:
            from app.scraper.source_validation import parse_multi_artist as _pma_rec
        ...
        for rec_candidate in recordings:
            ...
            if rel:
                ...
                priority = _RELEASE_TYPE_PRIORITY.get(rtype, 99)
                # De-prioritize releases whose release-group carries a
                # "Compilation" secondary type (VA samplers).
                stypes = [s.lower() for s in rg.get("secondary-type-list", [])]
                if "compilation" in stypes:
                    priority += 3
                if priority < best_priority:
                    ...
                    # Tiebreaker: prefer release whose title matches the
                    # search title (title-track heuristic — the artist's
                    # own album named after the song is more relevant
                    # than a third-party compilation that includes it).
                    _cur = (best_rel.get("title", "") if best_rel else "").lower().strip()
                    _new = rel.get("title", "").lower().strip()
                    if _new == _title_norm and _cur != _title_norm:
                        best_rec = rec_candidate
                        best_rel = rel
        ...
        if best_rel:
            ...
            # single's mb_release_id Ã¢â‚¬â€ the track is just an album track.
            if rel_type not in ("single", "ep") and result.get("mb_artist_id"):
                ...
                if confirmed_single:
                    # A single exists Ã¢â‚¬â€ use the confirmed single's
                    ...
                    # No matching single for this artist Ã¢â‚¬â€ this is just an
                    ...
                    logger.info(
                        ...
                        f"matching single exists for this artist Ã¢â‚¬â€ skipping "
                    ...
                    # below Ã¢â‚¬â€ we intentionally leave those unset for album-only
...
def search_wikipedia(title: str, artist: str) -> Optional[str]:
    ...
    """
    # Strip featuring suffixes Ã¢â‚¬â€ "(feat. X)", "(ft. X)", "(featuring X)"
    ...
    # Strip remix/version suffixes Ã¢â‚¬â€ "(Fatboy Slim Remix)", "(Acoustic Version)"
    ...
    # Extract primary artist from featuring credits so
    # "DJ Snake & Lil Jon" Ã¢â€ â€™ "DJ Snake" for better search results.
    from app.scraper.source_validation import parse_multi_artist as _pma_wiki_song
    _primary_artist, _ = _pma_wiki_song(artist)
    _search_artist = _primary_artist or artist

    search_terms = [
        f"{title} ({_search_artist} song)",
        f"{title} (song)",
        f"{_search_artist} {title} single",
        f"{_search_artist} {title}",
        f"{title} {_search_artist} song",
        f'"{title}" {_search_artist}',
        title,
    ]
    # Also include full artist name if different from primary
    if _search_artist.lower() != artist.lower():
        search_terms.insert(3, f"{artist} {title}")
    ...
    # Normalize Unicode hyphens (U+2010, U+2011, U+2013, U+2014, U+2212) Ã¢â€ â€™ ASCII
    ...
    # (e.g. "Florence + the Machine" Ã¢â€ â€ "Florence and the Machine")
    ...
    if "-" in artist_lower:
        ...
        _artist_variants.add(artist_lower.replace("-", " "))
    # Add primary artist variant for matching
    _primary_lower = _primary_artist.lower().strip() if _primary_artist else ""
    if _primary_lower and _primary_lower != artist_lower:
        _artist_variants.add(_primary_lower)
    ...
    for search_term in search_terms:
        ...
        for r in results:
            ...
            if title_lower and (title_lower in pt_lower or _title_norm in _pt_norm):
                score += 3
            # Dual A-side singles: "Song for Shelter/Ya Mama" Ã¢â‚¬â€ check if
            # our title matches one part of a "/" page title.
            elif title_lower and "/" in pt_lower:
                for _slash_part in pt_lower.split("/"):
                    _sp = _slash_part.strip()
                    _sp_norm = _normalize_title_for_match(_sp)
                    if _sp == title_lower or _sp_norm == _title_norm:
                        score += 3
                        break
            ...
            if title_lower and (_pt_base == title_lower or _pt_base_norm == _title_norm):
                score += 2
            # Dual A-side exact match: "Song for Shelter/Ya Mama" stripped
            # base is "song for shelter/ya mama" Ã¢â‚¬â€ check slash parts.
            elif title_lower and "/" in _pt_base:
                for _slash_part in _pt_base.split("/"):
                    _sp = _slash_part.strip()
                    _sp_norm = _normalize_title_for_match(_sp)
                    if _sp == title_lower or _sp_norm == _title_norm:
                        score += 2
            ...
            if _song_disambig:
                ...
                # penalise Ã¢â‚¬â€ e.g. "Here Without You (The Byrds song)" when
                ...
                if _disambig_text and artist_lower:
                    ...
                    if not _disambig_has_our_artist:
                        score -= 10  # Wrong artist's song
            if artist_lower and any(f"({av}" in pt_lower for av in _artist_variants):
                score += 2
            # De-score album pages Ã¢â‚¬â€ we want SONG pages, not album pages
            _album_page_tags = ["(album)", "(ep)", "(mixtape)"]
            if any(tag in pt_lower for tag in _album_page_tags):
                score -= 5
            # De-score when snippet strongly indicates album page
            _album_snippet_kw = [
                "studio album", "compilation album", "debut album",
                "second album", "third album", "fourth album",
                "is the album", "is an album",
            ]
            if any(w in snippet_lower for w in _album_snippet_kw):
                score -= 3
            ...
            # title Ã¢â‚¬â€ these are likely artist/bio pages, not song pages.
            # For dual A-side singles, check "/" parts too.
            _title_in_page = (
                title_lower in pt_lower
                or _title_norm in _pt_norm
                or ("/" in pt_lower and any(
                    title_lower == sp.strip() or _title_norm == _normalize_title_for_match(sp.strip())
                    for sp in pt_lower.split("/")
                ))
            )
            if (artist_lower and title_lower
                    and any(av in pt_lower for av in _artist_variants)
                    and not _title_in_page):
                score -= 2
            ...
            if any(w in snippet_lower for w in _non_music_snippet):
                score -= 3
            # Penalize when page title contains NEITHER the artist NOR the
            # song title Ã¢â‚¬â€ low relevance signals, likely a wrong page.
            # Also check the normalized title variant so that wordÃ¢â€ â€number
            # substitutions (e.g. "for"Ã¢â€ â€"4") don't trigger a false penalty.
            # For dual A-side singles, check "/" parts too.
            _title_found_in_pt = (
                title_lower in pt_lower
                or _title_norm in _pt_norm
            ...
            if (artist_lower and title_lower
                    and not any(av in pt_lower for av in _artist_variants)
                    and not _title_found_in_pt):
                score -= 4
    ...
    # Minimum score threshold Ã¢â‚¬â€ a score below 6 means neither the title
    ...
    # Ã¢â€â‚¬Ã¢â€â‚¬ Multi-phase verification Ã¢â€â‚¬Ã¢â€â‚¬
    ...
    for candidate in candidates:
        ...
        # Handle dual A-side singles: "Song for Shelter/Ya Mama"
        # where one part of the "/" matches our title exactly.
        if "/" in _clean_page and _best_sim < 0.6:
            for _part in _clean_page.split("/"):
                _part = _part.strip()
                if not _part:
                    continue
                _part_sim = SequenceMatcher(None, _clean_title_lower, _part.lower()).ratio()
                _part_norm = SequenceMatcher(None, _title_norm, _normalize_title_for_match(_part.lower())).ratio()
                _best_sim = max(_best_sim, _part_sim, _part_norm)
        if _best_sim < 0.6:
        ...
        if _scraped_artist and artist_lower:
            ...
            if not _artist_ok:
                logger.info(
                    f"Wikipedia: candidate '{candidate['title']}' (score={candidate['score']}) "
                    f"has artist '{_scraped_artist}' Ã¢â‚¬â€ doesn't match expected '{artist}', "
        ...
        if not _scraped_artist and artist_lower:
            ...
            if _plot and not any(av in _plot for av in _artist_variants):
                logger.info(
                    f"Wikipedia: candidate '{candidate['title']}' (score={candidate['score']}) "
                    f"Ã¢â‚¬â€ no infobox artist and '{artist}' not in first paragraph, "
                    f"trying next candidate"
                )
                _verified += 1
                if _verified >= _MAX_VERIFY:
...
def search_wikipedia_artist(artist: str) -> Optional[str]:
    ...
    from app.scraper.source_validation import parse_multi_artist as _pma_wiki
    _primary, _ = _pma_wiki(artist)
    if _primary and _primary.lower().strip() != artist_lower:
        _artist_variants.add(_primary.lower().strip())

    # For "X & The Y" / "X and The Y" band patterns, extract the lead artist
    # as a search variant.  parse_multi_artist deliberately keeps these as one
    # entity, but the Wikipedia page is often under just the lead artist name
    # (e.g. "Amanda Palmer" not "Amanda Palmer & The Grand Theft Orchestra").
    _lead_match = re.match(r'^(.+?)\s*(?:&|and)\s+the\s+', artist, re.IGNORECASE)
    if _lead_match:
        _lead = _lead_match.group(1).strip()
        if _lead and _lead.lower() != artist_lower:
            _artist_variants.add(_lead.lower())

    # Use primary artist for search terms (better Wikipedia results).
    # Prefer the lead-artist extraction for "& The" patterns.
    _search_name = _lead if _lead_match else (_primary if _primary and _primary.lower() != artist_lower else artist)

    search_terms = [
        f"{_search_name} (band)", f"{_search_name} (musician)",
        f"{_search_name} (singer)", f"{_search_name} (rapper)", _search_name,
    ]
    # Also include full-name search terms when lead artist differs
    if _search_name.lower() != artist_lower:
        search_terms.extend([f"{artist} (band)", artist])
    ...
    for term in search_terms:
        for r in _wikipedia_search_api(term, limit=5):
            ...
            # Artist name MUST appear in page title Ã¢â‚¬â€ otherwise it's a
            ...
            # contains extra words beyond the artist name Ã¢â‚¬â€ likely a
            # different entity (e.g. "Blackshape Prime" for "Blackshape",
            # "A. B. Lucas Secondary School" for "B. Lucas").
            ...
            # "YOLO (The Lonely Island song)" Ã¢â‚¬â€ literal "(song)" won't match.
...
def search_wikipedia_album(artist: str, album: str) -> Optional[str]:
    ...
    for term in search_terms:
        for r in _wikipedia_search_api(term, limit=5):
            ...
            if _album_disambig:
                score += 3
                # Check if the disambiguation names an artist
                _ad_text = _album_disambig.group(1).strip()
                # Common qualifiers that are NOT artist names
                _ALBUM_QUALIFIERS = {
                    "debut", "live", "compilation", "deluxe", "self-titled",
                    "soundtrack", "remix", "greatest hits", "extended",
                ...
                if artist and _ad_text and not _is_qualifier and artist.lower() in _ad_text:
                    ...
                    # artist's album (e.g. "Mr. Big album" when searching
                    # for "Luca Stricagnoli").
                    score -= 10  # Wrong artist's album
...
def scrape_wikipedia_page(url: str, expected_artist: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        # Detect disambiguation pages Ã¢â‚¬â€ they have no useful music metadata
        _body = soup.find("body")
        if _body and "mw-disambig" in (_body.get("class") or []):
            result["page_type"] = "disambiguation"
            logger.info(f"Wikipedia: disambiguation page detected Ã¢â‚¬â€ {url}")
            return result
        ...
        from app.scraper.source_validation import classify_wikipedia_page
...
def detect_article_mismatch(
    ...
    if page_type == "disambiguation":
        return "disambiguation page Ã¢â‚¬â€ not a music article"
...
def _extract_infobox_artist(infobox) -> Optional[str]:
    ...
    """
    try:
        # Method 1: infobox-subheader Ã¢â‚¬â€ "Single by Artist" / "Song by Artist"
        ...
        # Method 2: description headers Ã¢â‚¬â€ "... by Artist"
    ...
    try:
        ...
        for table in soup.find_all("table", {"class": "tracklist"}):
            for row in table.find_all("tr"):
                ...
                if not tds:
                    continue
                # Title is typically the first <td> that contains text
                # (after the row-header <th> with track number)
                title_td = tds[0]
...
def extract_wiki_infobox_links(page_url: str) -> dict:
    """Extract album and artist Wikipedia URLs from a page's infobox in one fetch.

    Combines the logic of extract_album_wiki_url_from_single and
    extract_artist_wiki_url_from_page into a single HTTP round-trip for
    use in cross-link confirmation and fallback.

    Returns {"album_url": str|None, "artist_url": str|None}.
    """
    result = {"album_url": None, "artist_url": None}
    if not page_url:
        return result
    try:
        headers = {"User-Agent": _WIKI_USER_AGENT, "Api-User-Agent": _WIKI_USER_AGENT}
        ...
        # --- Album link ("from the album ...") ---
        for elem in infobox.find_all(["th", "td"], {"class": "infobox-header"}):
            text = elem.get_text(strip=True).lower()
            if "album" in text:
                a_tag = elem.find("a", href=True)
                if a_tag and a_tag["href"].startswith("/wiki/"):
                    result["album_url"] = f"https://en.wikipedia.org{a_tag['href']}"
                    break
        if not result["album_url"]:
            for th in infobox.find_all("th"):
                label = th.get_text(strip=True).lower()
                if label in ("album", "from the album", "from"):
                    td = th.find_next_sibling("td")
        ...
        # Reject generic music-concept articles (e.g. "Album", "EP")
        # that appear as type labels on album pages, not actual album links.
        if result["album_url"]:
            _page = result["album_url"].rsplit("/wiki/", 1)[-1]
            _GENERIC = frozenset({
                "Album", "Single_(music)", "Extended_play", "Studio_album",
                "Compilation_album", "Live_album", "Remix_album",
                "Soundtrack_album", "Mixtape", "EP", "Demo_(music)",
                "Bootleg_recording", "Soundtrack",
            })
            if _page in _GENERIC:
                result["album_url"] = None

        # --- Artist link ("by <Artist>") ---
        for elem in infobox.find_all(["td", "th"], {"class": ["infobox-subheader", "infobox-header"]}):
            full_html = str(elem)
            by_idx = full_html.lower().find(" by ")
            if by_idx < 0:
                continue
            after_by = BeautifulSoup(full_html[by_idx + 4:], "html.parser")
            for a_tag in after_by.find_all("a", href=True):
                href = a_tag.get("href", "")
                if href.startswith("/wiki/") and ":" not in href[6:]:
                    result["artist_url"] = f"https://en.wikipedia.org{href}"
                    break
            if result["artist_url"]:
                break

        # --- MusicBrainz release-group links ---
        # Scan all <a> tags for links to musicbrainz.org/release-group/UUID.
        # Wikipedia pages often list MB links in the "External links" section
        # or in authority-control navboxes at the bottom.
        _mb_rg_re = re.compile(
        ...
        for a_tag in soup.find_all("a", href=True):
            m = _mb_rg_re.search(a_tag["href"])
            if m and m.group(1) not in _mb_rgs:
                _mb_rgs.append(m.group(1))
        if _mb_rgs:
            result["mb_release_group_ids"] = _mb_rgs

        if result["album_url"]:
            logger.info(f"Wiki infobox link: album Ã¢â€ â€™ {result['album_url']}")
        if result["artist_url"]:
            logger.info(f"Wiki infobox link: artist Ã¢â€ â€™ {result['artist_url']}")
        if _mb_rgs:
            logger.info(f"Wiki MB links: release-groups Ã¢â€ â€™ {_mb_rgs}")
    except Exception as e:
        logger.debug(f"Wiki infobox link extraction failed for {page_url}: {e}")
    return result
...
def _extract_infobox_image(infobox, page_url: str) -> Optional[str]:
    ...
    try:
        for img in infobox.find_all("img"):
            ...
            # Get the original file URL Ã¢â‚¬â€ thumbnail resizing fails for
...
def _extract_plot_and_mv_info(soup) -> Optional[str]:
    ...
    # Use mw-parser-output (article body) Ã¢â€ â€™ mw-content-container Ã¢â€ â€™ fallback
    content = soup.find("div", {"class": "mw-parser-output"})
    if not content:
    ...
    # Music video section Ã¢â‚¬â€ iterate headings and check get_text()
    mv_heading = None
...
def resolve_metadata(
    ...
    if ytdlp_metadata:
        ...
        # NOTE: Do NOT set image_url from yt-dlp here Ã¢â‚¬â€ only scrapers should
    ...
    try:
        ...
        if mb.get("mb_recording_id"):
            ...
            # Prefer MB canonical names Ã¢â‚¬â€ but validate they're not wildly
            ...
            if mb.get("artist"):
                ...
                if (mb_artist_norm == input_artist_norm
                    ...
                    logger.warning(
                        ...
                        f"input '{metadata['artist']}' Ã¢â‚¬â€ keeping input artist, "
                        f"discarding MB IDs"
                    )
    ...
    if not skip_wikipedia:
        try:
            ...
            if wiki_url:
                ...
                if mismatch:
                    logger.warning(
                        f"Wikipedia article mismatch Ã¢â‚¬â€ discarding: {mismatch} "
                        f"(url={wiki_url})"
                    )
...
def download_image(url: str, save_path: str) -> bool:
    ...
    **DEPRECATED** Ã¢â‚¬â€ Callers should use ``artwork_service.download_and_validate()``
    ...
    import warnings
    warnings.warn(
        "download_image() is deprecated Ã¢â‚¬â€ use artwork_service.download_and_validate() directly",
        DeprecationWarning,
        stacklevel=2,
    )
    from app.pipeline_url.services.artwork_service import download_and_validate
