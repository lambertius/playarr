...
"""
Artist & Album Scraper Ã¢â‚¬â€ Retrieve artwork and metadata for Kodi's
Artist and Album browsing views.

Sources (in priority order):
1. MusicBrainz + Cover Art Archive Ã¢â‚¬â€ MBID-based artwork (highest quality)
2. Wikipedia Ã¢â‚¬â€ artist pages, album pages (infobox images, fallback)

The module is intentionally stateless Ã¢â‚¬â€ it returns image URLs and
...
def _resolve_commons_url(url: str) -> Optional[str]:
    ...
    if not url or "commons.wikimedia.org/wiki/File:" not in url:
        return url  # Not a Commons file page Ã¢â‚¬â€ return as-is
    ...
    if infobox:
        ...
        for img in images[1:]:  # Skip the first (already used as poster)
            ...
            if src and ("logo" in src.lower() or "banner" in src.lower()):
                continue
            # Skip SVG images (rendered as PNG thumbnails) — they are
            # Wikipedia UI icons, signatures, or wordmarks
            if ".svg" in src.lower():
                continue
            if src.startswith("//"):
                src = "https:" + src
...
def search_album_wikipedia(album: str, artist: str) -> Optional[str]:
    ...
    # Similarity gate: iterate candidates and require the cleaned title
    ...
    for cand in candidates:
        ...
        sim = _SM(None, album_lower, clean_title.lower()).ratio()
        if sim < 0.7:
            logger.info(
                f"Album Wikipedia: '{cand['title']}' title similarity "
...
def scrape_album_artwork(album: str, artist: str, *, wiki_url: Optional[str] = None) -> Dict[str, Any]:
    ...
    Returns dict:
        image_url: str | None   Ã¢â‚¬â€œ album cover URL
        year: int | None
        genres: list[str]
    """
    result: Dict[str, Any] = {
...
def search_artist_musicbrainz(artist: str, mb_artist_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    name-based search returns the wrong artist (e.g. "The Rions" Ã¢â€ â€™ "The
    Beatles").
    ...
    """
    _init_musicbrainz()
    result: Dict[str, Any] = {"mb_artist_id": None, "image_url": None}
    ...
    try:
        ...
        # Normalize Unicode hyphens so "aÃ¢â‚¬Âha" (U+2010) matches "a-ha" (ASCII)
        ...
        # MusicBrainz doesn't directly serve artist images Ã¢â‚¬â€ try URL relations
...
# ---------------------------------------------------------------------------
# Combined lookup (MusicBrainz first Ã¢â€ â€™ Wikipedia fallback)
# ---------------------------------------------------------------------------
...
def get_artist_artwork_musicbrainz(artist: str, mb_artist_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    MusicBrainz does not host artist images natively Ã¢â‚¬â€ its URL relations
    ...
    # Intentionally skip mb["image_url"] Ã¢â‚¬â€ it's a Wikimedia Commons link,
...
def get_artist_artwork(artist: str, mb_artist_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    # Extract primary artist for multi-artist strings (e.g. "AronChupa & Little Sis Nora" Ã¢â€ â€™ "AronChupa")
    from app.scraper.source_validation import parse_multi_artist
    primary, _ = parse_multi_artist(artist)
    if primary != artist:
        logger.info(f"Multi-artist detected: {artist!r} Ã¢â€ â€™ using primary: {primary!r}")
        artist = primary
...
def get_album_artwork_wikipedia(album: str, artist: str, *, wiki_url: Optional[str] = None) -> Dict[str, Any]:
    ...
    if not result["image_url"] and not result["year"]:
        ...
        if base_name:
            logger.info(f"Album '{album}' not found on Wikipedia Ã¢â‚¬â€ retrying as '{base_name}'")
...
def get_album_artwork_musicbrainz(album: str, artist: str) -> Dict[str, Any]:
    ...
    if not result["image_url"]:
        base_name = _strip_edition_suffix(album)
        if base_name:
            logger.info(f"Album '{album}' not found on CAA Ã¢â‚¬â€ retrying as '{base_name}'")
...
def get_album_artwork(album: str, artist: str, *, wiki_url: str | None = None) -> Dict[str, Any]:
    ...
    if not result["image_url"] and not result["year"]:
        base_name = _strip_edition_suffix(album)
        if base_name:
            logger.info(f"Album '{album}' not found Ã¢â‚¬â€ retrying as '{base_name}'")
            _search(base_name)
