...
def get_or_create_artist(db, name: str, resolved: Optional[Dict] = None) -> ArtistEntity:
    ...
    if entity:
        ...
        if resolved:
            ...
            if resolved.get("field_provenance"):
                existing_prov = entity.field_provenance or {}
                existing_prov.update(resolved["field_provenance"])
                entity.field_provenance = existing_prov
        return entity
    ...
    entity = ArtistEntity(
        ...
        field_provenance=resolved.get("field_provenance"),
    )
    db.add(entity)
    db.flush()
