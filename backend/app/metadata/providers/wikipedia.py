...
class WikipediaProvider(MetadataProvider):
    ...
    def search_artist(self, name: str) -> List[ProviderResult]:
        ...
        # places) will have infoboxes with completely different fields.
        if infobox:
            _music_indicators = {
            ...
            for _th in infobox.find_all("th"):
                if _th.get_text(strip=True).lower() in _music_indicators:
                    ...
                    break
            # "occupation" can appear on non-music pages too — double-check
            # its value contains a music-related word.
