...
def infer_source_type_from_url(url: str, provider: str = "") -> str:
    ...
    - MusicBrainz artist URLs → "artist"
    - MusicBrainz release-group URLs → "single"
    - MusicBrainz recording URLs → "recording"
    - MusicBrainz release URLs → "album" (unless looks like single)
    - Wikipedia artist pages → "artist"
    - Wikipedia album pages → "album"
    - Wikipedia song/single pages → "single"
    """
    url_lower = url.lower()
...
_FEAT_PATTERNS = [
    ...
    r'\s*\(featuring\s+',  # "(featuring X)" — parenthesized
    r'\s*\(ft\.?\s+',     # "(ft. X)" — parenthesized
    r'\s+feat\.?\s+',
    r'\s+featuring\s+',
    r'\s+ft\.?\s+',
    r'\s+with\s+',
...
def parse_multi_artist(artist_string: str) -> Tuple[str, List[str]]:
    ...
    Examples:
        "AronChupa feat. Little Sis Nora" → ("AronChupa", ["Little Sis Nora"])
        "AronChupa and Little Sis Nora" → ("AronChupa", ["Little Sis Nora"])
        "AronChupa & Little Sis Nora" → ("AronChupa", ["Little Sis Nora"])
        "DJ Snake & Lil Jon" → ("DJ Snake", ["Lil Jon"])
        "Florence + the Machine" → ("Florence + the Machine", [])
        "Mike Diva & Sick System" → ("Mike Diva", ["Sick System"])
...
def _split_artists(artist_str: str) -> List[str]:
    ...
    Protected patterns:
    - "X + the Y" — band name (Florence + the Machine)
...
def sanitize_album(album: Optional[str], title: str = "") -> Optional[str]:
    ...
    # values in — e.g. '"I\'m an Albatraoz - Single"'. These prevent
    ...
    # the field blank — and there are real albums called "Unknown" etc.
