...
def compute_expected_paths(
    ...
    """
    Compute the expected folder path, file path, and file base name for a video
    based on the current naming convention settings.

    Returns dict with keys: folder_path, file_path, file_base_name, subpath
    """
    file_base_name = build_folder_name(
        artist, title, resolution_label,
        version_type=version_type,
        alternate_version_label=alternate_version_label,
    )
    ...
    folder_path = os.path.join(library_dir, subpath)
    file_path = os.path.join(folder_path, f"{file_base_name}{file_ext}")
