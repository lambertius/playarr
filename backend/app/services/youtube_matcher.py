...
def verify_youtube_link(
    ...
    """Check whether an existing YouTube URL is a valid match for the given track.

    Uses yt-dlp to fetch metadata for the specific video, then scores it
    against the known artist/title/duration.  Returns a YouTubeCandidate if the
    video exists and meets the confidence threshold, otherwise ``None``.
    """
    settings = get_settings()
    ytdlp = settings.resolved_ytdlp
    ...
    if result.returncode != 0:
        logger.info(f"YouTube verify: video unavailable at {url}")
        return None
    ...
    # Score against known metadata (same logic as search_youtube)
    search_target = _strip_title_tags(f"{artist} - {title}").lower()
    candidate_title_stripped = _strip_title_tags(candidate.title).lower()
    candidate.title_score = SequenceMatcher(
        None, search_target, candidate_title_stripped
    ...
    if duration_seconds and candidate.duration_seconds:
        diff = abs(duration_seconds - candidate.duration_seconds)
        tolerance = duration_seconds * 0.15
        if diff <= tolerance:
            candidate.duration_score = 1.0 - (diff / (tolerance * 2))
        elif diff <= tolerance * 2:
            candidate.duration_score = 0.25
    ...
    candidate.overall_score = (
        candidate.title_score * 0.55 +
        candidate.duration_score * 0.40 +
        0.05  # small boost for being a known link
    )

    if candidate.overall_score >= min_confidence:
        logger.info(f"Verified YouTube link {url}: score={candidate.overall_score:.3f}")
        return candidate

    logger.info(f"YouTube link {url} below threshold {min_confidence} (score={candidate.overall_score:.3f})")
    return None
