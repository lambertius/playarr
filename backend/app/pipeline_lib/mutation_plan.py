# AUTO-SEPARATED from pipeline/mutation_plan.py for pipeline_lib pipeline
...
def build_plan_from_workspace(ws) -> dict:
    ...
    # Override ratings from trusted Playarr XML if available
    xml_ratings = ws.read_artifact("xml_ratings") or {}
    if xml_ratings.get("song_rating") is not None:
        plan["video"]["song_rating"] = xml_ratings["song_rating"]
    if xml_ratings.get("video_rating") is not None:
        plan["video"]["video_rating"] = xml_ratings["video_rating"]
    ...
    plan["video"]["mb_release_group_id"] = metadata.get("mb_release_group_id")
    ...
    elif user_action == "keep_both":
        forced_vt = dup_check.get("forced_version_type")
        if forced_vt:
            plan["version_type"] = forced_vt
    elif user_action == "review_later":
        plan["review_status"] = "needs_human_review"
        plan["review_reason"] = "User deferred to review"
    ...
    # NOTE: track_identified and canonical_linked are set dynamically in
    # db_apply after entity creation actually succeeds, not here.
    ...
    if input_data.get("import_type") == "url":
        ...
        flags["imported"] = "import"
    elif input_data.get("import_type") == "library":
        flags["downloaded"] = "library"
        flags["imported"] = "library"

    # Merge processing state from trusted Playarr XML
    xml_processing = ws.read_artifact("xml_processing_state") or {}
    for step_name, step_info in xml_processing.items():
        if step_info.get("completed") and step_name not in flags:
            flags[step_name] = step_info.get("method") or "playarr_xml"
    ...
    if mode == "advanced":
        ...
        _ai_requested = bool(opts.get("ai_auto_analyse") or opts.get("ai_auto_fallback"))
