# AUTO-SEPARATED from ai/metadata_service.py for pipeline_lib pipeline
...
def _auto_apply_fields(
    ...
    if (
        ...
        _clean_title = ai_result.ai_title
        # Guard: strip version-type suffixes the AI may add to the title.
        # The version_type field already captures cover/live/alternate;
        # the title should be the clean song name.
        if video.version_type in ("cover", "live", "alternate"):
            import re as _re_title
            _clean_title = _re_title.sub(
                r'\s*\([^)]*\b(?:cover(?:ed)?|in the style of|live at|live from|remix)\b[^)]*\)\s*$',
                '', _clean_title, flags=_re_title.IGNORECASE,
            ).strip()
            if not _clean_title:
                _clean_title = ai_result.ai_title  # fallback if stripping emptied it
            elif _clean_title != ai_result.ai_title:
                logger.info(
                    f"AI title sanitized for {video.version_type}: "
                    f"'{ai_result.ai_title}' → '{_clean_title}'"
                )
        video.title = _clean_title
    ...
    if (
        ...
        # Guard: for covers, never accept AI-suggested albums.
        # AI almost always returns the *original* artist's album (e.g.
        # "69 Love Songs" for a piano cover of The Magnetic Fields' song),
        # which is a false positive for the cover artist.
        _is_cover = video.version_type == "cover"
        ...
        if _is_compilation:
            ...
            logger.info(
                f"AI album rejected (cover): '{ai_result.ai_album}' "
                f"for video {video.id} — covers should not inherit "
                f"the original artist's album"
            )
        else:
    ...
    if needs_entity_resolve:
        # When AI changed the artist, clear stale MusicBrainz IDs that were
        # resolved under the old artist identity.  Without this, entity
        # re-resolution would use the original artist's MB IDs (e.g. The
        # Magnetic Fields' artist ID for a B. Lucas cover).
        if "artist" in applied:
            _stale_mb = []
            for _mbf in ("mb_artist_id", "mb_recording_id", "mb_release_id",
                         "mb_release_group_id"):
                if getattr(video, _mbf, None):
                    _stale_mb.append(f"{_mbf}={getattr(video, _mbf)}")
                    setattr(video, _mbf, None)
            if _stale_mb:
                logger.info(
                    f"Cleared stale MB IDs after AI artist change for "
                    f"video {video.id}: {', '.join(_stale_mb)}"
                )
...
def apply_ai_fields(
    ...
    if requested_artwork:
        ...
        for art_type in requested_artwork:
            ...
            if old_valid:
                if old_valid.file_path and _os.path.isfile(old_valid.file_path):
                    try:
                        _os.remove(old_valid.file_path)
                    except OSError:
                        pass
                db.delete(old_valid)
            # Rename pending file to canonical name
                ...
                if pending.file_path != canonical:
                    try:
                        if _os.path.isfile(pending.file_path):
                            _os.replace(pending.file_path, canonical)
        ...
        for ps in ai_result.proposed_sources:
            ...
            if existing:
                ...
                if ps.get("_replaces_source_id") and existing.id == ps["_replaces_source_id"]:
                    ...
                    existing.original_url = ps["original_url"]
                    existing.canonical_url = ps["original_url"]
                continue
            try:
...
def compare_metadata(
    ...
    # Find pending artwork proposed by this scrape, paired with current valid artwork
    artwork_updates = []
    pending_assets = (
        db.query(MediaAsset)
        .filter(
