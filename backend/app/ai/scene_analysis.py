...
def analyze_scenes(
    ...
    if not force:
            ...
            if not has_selected:
                ...
                if best:
                    ...
                    if video.review_category in ("missing_artwork", "artwork_incomplete"):
                        ...
                        # Thumbnail is now selected, so check poster only if it was flagged
                        if not needs_poster:
        ...
        # Clean up old thumbnails and analysis records before re-running
        old_thumbs = db.query(AIThumbnail).filter(AIThumbnail.video_id == video_id).all()
        for ot in old_thumbs:
            if ot.file_path and os.path.isfile(ot.file_path):
                try:
                    os.remove(ot.file_path)
                except OSError:
                    pass
            db.delete(ot)
    ...
    try:
        ...
        # AI-assisted re-ranking (if enabled)
        if thumbnails:
            thumbnails = _maybe_ai_rerank(db, video, thumbnails)
        ...
        if thumbnails:
            ...
            # Mark thumbnail_selected processing flag
            from sqlalchemy.orm.attributes import flag_modified
            state = dict(video.processing_state or {})
            state["thumbnail_selected"] = {
                "completed": True,
...
def select_thumbnail(
    ...
    *,
    ...
    Args:
        ...
        apply_to_poster: Also set this thumbnail as the poster artwork
    ...
    if video:
        ...
        if apply_to_poster:
            _save_as_poster(db, video, thumb)

        # Mark the thumbnail_selected processing flag
        from sqlalchemy.orm.attributes import flag_modified
        state = dict(video.processing_state or {})
        state["thumbnail_selected"] = {
            "completed": True,
...
# ---------------------------------------------------------------------------
# AI-assisted re-ranking
# ---------------------------------------------------------------------------

def _maybe_ai_rerank(
    db: Session,
    video: VideoItem,
    thumbnails: List[AIThumbnail],
) -> List[AIThumbnail]:
    """
    Optionally re-rank thumbnails using AI vision when scene_analysis_mode
    is set to 'ai_assisted'.  Blends heuristic scores (40%) with AI vision
    scores (60%) so the AI judgment dominates but heuristics still break ties.

    Returns the same list with score_overall updated if AI ranking succeeded.
    """
    from app.models import AppSetting

    # Check if AI-assisted mode is enabled
    row = db.query(AppSetting).filter(
        AppSetting.key == "ai_scene_mode",
        AppSetting.user_id.is_(None),
    ).first()
    mode = row.value if row else "heuristic"

    if mode != "ai_assisted":
        return thumbnails

    logger.info(f"AI-assisted thumbnail ranking for video {video.id}")
    ...
    try:
        ...
        selection = router.select_model(TaskType.SCENE_RANKING)
        provider = get_ai_provider(
            provider_name=selection.provider,
            model=selection.model,
        )
        if not provider:
            logger.warning("AI provider not available for thumbnail ranking")
            return thumbnails

        # Gather image paths
        image_paths = [t.file_path for t in thumbnails if t.file_path and os.path.isfile(t.file_path)]
        if not image_paths:
            return thumbnails
        ...
        rankings = provider.rank_thumbnails(image_paths, artist=artist, title=title)
        if not rankings:
            logger.info("AI ranking returned no results, keeping heuristic scores")
            return thumbnails

        # Build index→AI score lookup
        ai_scores = {}
        for r in rankings:
            if 0 <= r.index < len(thumbnails):
                ai_scores[r.index] = r
        ...
        for i, thumb in enumerate(thumbnails):
            if i in ai_scores:
                ai = ai_scores[i]
                blended = thumb.score_overall * 0.4 + ai.score * 0.6
                # Penalize blurry / text-heavy frames
                if ai.is_blur:
                    blended *= 0.3
                if ai.has_text:
                    blended *= 0.7
                thumb.score_overall = round(min(blended, 1.0), 3)
                thumb.provenance = "ai_ranked"

        logger.info(f"AI re-ranked {len(ai_scores)} of {len(thumbnails)} thumbnails")
        return thumbnails

    except Exception as e:
        logger.error(f"AI thumbnail re-ranking failed, keeping heuristic scores: {e}")
        return thumbnails
...
def _save_as_video_thumb(db: Session, video: VideoItem, thumb: AIThumbnail):
    ...
    from app.services.artwork_service import validate_file as _validate_thumb
...
def _save_as_poster(db: Session, video: VideoItem, thumb: AIThumbnail):
    """Save a thumbnail as both the poster and thumb MediaAsset records."""
    from app.services.artwork_service import validate_file as _validate_thumb
    _vr = _validate_thumb(thumb.file_path) if os.path.isfile(thumb.file_path) else None
    _prov_fields = dict(
        provenance="manual_thumbnail",
        status="valid" if (_vr and _vr.valid) else "invalid",
        width=_vr.width if _vr and _vr.valid else None,
        height=_vr.height if _vr and _vr.valid else None,
        file_size_bytes=_vr.file_size_bytes if _vr and _vr.valid else None,
