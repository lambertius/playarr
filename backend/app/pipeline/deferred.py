...
from app.pipeline.db_apply import _apply_lock
    ...
    try:
        ...
        if item.mb_recording_id and not item.mb_release_group_id:
            try:
                ...
                from app.services.source_validation import parse_multi_artist as _pma_c
                ...
                if _mb_c.get("mb_recording_id") and _mb_c.get("mb_release_group_id"):
                    ...
                    if (_mb_n == _exp_n or _mb_n == _pri_n
                        ...
                        if _mb_c.get("album") and item.artist_entity:
                            from app.metadata.resolver import get_or_create_album as _goca_c
                            _alb_resolved = {
        ...
        if (item.mb_release_group_id and item.album_entity
            ...
            try:
                from app.services.metadata_resolver import search_musicbrainz as _mb_1d
        ...
        if (item.mb_recording_id and item.album_entity
            ...
            try:
                from app.services.metadata_resolver import search_musicbrainz as _mb_1dp
...
def _re_resolve_sources(db, video_id: int, artist: str, title: str,
    ...
    # Delete stale scraped/ai sources (wikipedia, musicbrainz, imdb) so they
    # can be re-resolved from scratch.  Keep import/matched/manual sources.
    stale = (
        db.query(Source)
        .filter(
    ...
    if album:
        try:
            from app.services.metadata_resolver import search_wikipedia_album
    ...
    _xr_ws = locals().get("ws_url")
    _xr_wl = locals().get("wl_url")
    if not _xr_ws and _xr_wl:
        try:
            from app.services.metadata_resolver import extract_single_wiki_url_from_album
            _tl_url = extract_single_wiki_url_from_album(_xr_wl, title)
            if _tl_url:
    ...
    if item:
        ...
        mb_rg = getattr(item, "mb_release_group_id", None)
        _used_mb_rg_ids: set = set()
        if mb_rg:
            new_sources.append(Source(
                video_id=video_id, provider="musicbrainz",
                source_video_id=mb_rg,
                original_url=f"https://musicbrainz.org/release-group/{mb_rg}",
                canonical_url=f"https://musicbrainz.org/release-group/{mb_rg}",
                source_type="single", provenance="ai",
            ))
            _used_mb_rg_ids.add(mb_rg)
...
def _re_organize_file(db, video_id: int, ws: ImportWorkspace) -> None:
    ...
    from app.services.file_organizer import build_folder_name, sanitize_filename
