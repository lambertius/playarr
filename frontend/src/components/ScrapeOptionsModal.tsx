/**
 * ScrapeOptionsModal — Shows scrape options (metadata sources, normalize)
 * before batch-scraping review queue items.
 */
import { useState, useEffect } from "react";
import ReactDOM from "react-dom";
import { X, ScanSearch, Info } from "lucide-react";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
        ...
        <p className="text-sm text-text-muted mb-4">
          Configure scrape options for {itemCount} item{itemCount !== 1 ? "s" : ""}
        </p>
        ...
        <div className="space-y-4">
          ...
          <button onClick={handleSubmit} disabled={isPending} className="btn-primary w-full">
            <ScanSearch size={16} className="mr-1" />
            {isPending ? "Queuing..." : `Scrape ${itemCount} Video${itemCount !== 1 ? "s" : ""}`}
          </button>
        </div>
      </div>
    </div>,
